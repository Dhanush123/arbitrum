...
class ReadOnlyTransaction {
   ...
   public:
    ...
    void enableReadSnapshot();
    void disableReadSnapshot();

    rocksdb::Status defaultGet(const rocksdb::Slice& key,
                               std::string* value) const;
    rocksdb::Status stateGet(const rocksdb::Slice& key,
                             std::string* value) const;
    rocksdb::Status blocksGet(const rocksdb::Slice& key,
                              std::string* value) const;
    rocksdb::Status nodeGet(const rocksdb::Slice& key,
                            std::string* value) const;
    ...
    [[nodiscard]] std::unique_ptr<rocksdb::Iterator> blocksGetIterator() const;
    [[nodiscard]] std::unique_ptr<rocksdb::Iterator> nodeGetIterator() const;
    ...
    [[nodiscard]] ValueResult<uint256_t> stateGetUint256(
        rocksdb::Slice key_slice) const;
    [[nodiscard]] ValueResult<uint256_t> blocksGetUint256(
        rocksdb::Slice key_slice) const;
    [[nodiscard]] ValueResult<uint256_t> nodeGetUint256(
        rocksdb::Slice key_slice) const;
